...
pub fn parse_auth_header(auth_string: &str) -> Result<AuthHeader, s3_core::S3Error> {
...
fn parse_credential_header(credential_string: &str) -> Result<Credential, s3_core::S3Error> {
    ...
    let date = chrono::NaiveDate::parse_from_str(date, "%Y%m%d")
        .map_err(|_| s3_core::S3Error::AuthorizationHeaderMalformed)?
        .and_hms_opt(0, 0, 0)
        .unwrap();
...
pub fn parse_date(headers: &HeaderMap) -> Result<chrono::NaiveDateTime, s3_core::S3Error> {
...
fn get_payload_hash(req: &Request<axum::body::Bytes>) -> String {
...
mod tests {
    ...
    fn test_put_object() {
        let req = Request::builder()
            ...
            .uri("/test$file.text")
            .body(axum::body::Bytes::from("Welcome to Amazon S3."))
            .unwrap();
